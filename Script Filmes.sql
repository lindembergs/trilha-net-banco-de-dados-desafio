...
SELECT [AnoLancamento], COUNT(*) AS QuantidadeFilmes
FROM [Filmes]
...
FROM [Filmes] F
JOIN [Generos] G ON F.[GeneroID] = G.[GeneroID];
...
FROM [Filmes] F
JOIN [FilmeAtores] FA ON F.[FilmeID] = FA.[FilmeID]
JOIN [Atores] A ON FA.[AtorID] = A.[AtorID];
